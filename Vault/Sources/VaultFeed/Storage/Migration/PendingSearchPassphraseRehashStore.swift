...
/// Persisted as a sibling of the SwiftData store during the V2 → V3 schema
/// migration. Records the plaintext search passphrases that were dropped
/// from the schema in Phase A so they can be hashed and written back in
/// Phase B (after the vault key becomes available on first post-update
/// unlock).
///
...
/// the user's first unlock that triggers `SearchPassphraseRehashService.run`.
///
...
struct PendingSearchPassphraseRehashStore: @unchecked Sendable { // swiftlint:disable:this no_unchecked_sendable
    ...
    static func defaultURL(storeDirectory: URL) -> URL {
        storeDirectory.appending(path: "vault-primary.pending-search-passphrase-rehash.json")
    }
